...
const navItems = [
  ...
  { icon: Waves, label: 'Hazard Detection', id: 'hazards' },
  { icon: Shield, label: 'Bio-Kinematics', id: 'bio' },
  { icon: LifeBuoy, label: 'Rescue Deployment', id: 'rescue' },
];

export function Sidebar({ activeTab, setActiveTab }: { activeTab: string, setActiveTab: (id: string) => void }) {
  return (
    <aside className="w-64 border-r border-guardian-slate bg-guardian-navy h-screen flex flex-col z-20">
